let users = [{
  ...
  // конструктор
function UsersList(usersArray) {
  this.usersArray = usersArray;
}  
 
  UsersList.prototype.showNames = function() {
  ...
  UsersList.prototype.showById = function(id) {
...
UsersList.prototype.add = function(newUser) {
  if(!newUser.first_name){
    console.log('cannot add new user because..');
  return;
  }    
...
UsersList.prototype.getMaxId = function() {    
  let maxId = -1;
  for(let i = 0;i<this.usersArray.length;i++){
...
UsersList.prototype.remove = function(id) {
  let filteredArray = this.usersArray.filter(function(obj) {
    ...
    if(filteredArray.length > 0){
      let index = this.usersArray.indexOf(filteredArray[0]);
...
UsersList.prototype.logUsersCount = function() {
...
let userList = new UsersList(users); // new instance
